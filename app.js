...
function renderOperationalTabs() {
  const tabs = ["Visão geral", "Checklist", "Tarefas", "Comunicação", "Convidados", "Fornecedores", "Documentos", "Histórico"];
  return tabs
    .map((tab, index) => `
      <button class="whitespace-nowrap border-b-2 pb-3 text-sm font-medium ${index === 0 ? "border-primary text-primary" : "border-transparent text-on-surface-variant"}">
        ${tab}
      </button>
    `)
    .join("");
...
function renderEventDetail(event) {
  ...
  const locationText = event.locations?.length ? event.locations.join(", ") : "Local a definir";
  const timeText = [event.startTime, event.endTime].filter(Boolean).join(" - ") || "Horário a definir";
  const communication = formatStatusLabel(event.communicationStatus);
    ...
    <div class="mb-6 overflow-hidden rounded-lg border border-outline-variant bg-primary-container">
      <div class="flex min-h-28 items-end justify-between gap-4 bg-primary-container px-6 py-5 text-white">
        ...
        <div class="flex gap-2">
          <button class="inline-flex items-center gap-2 rounded border border-white/25 bg-white/10 px-3 py-2 text-sm font-medium hover:bg-white/20">
            <span class="material-symbols-outlined text-base">edit</span>
            Editar
          </button>
          <button class="rounded border border-white/25 bg-white/10 px-3 py-2 hover:bg-white/20">
    ...
    <div class="grid grid-cols-1 gap-6 xl:grid-cols-[1fr_300px]">
      <div class="space-y-6">
        <section class="rounded-lg border border-outline-variant bg-white p-6">
          <div class="grid grid-cols-1 gap-4 md:grid-cols-4">
            <div>
              <p class="text-xs font-medium text-on-surface-variant">Data</p>
              <p class="mt-1 font-medium">${formatDateRange(event)}</p>
            </div>
            <div>
              <p class="text-xs font-medium text-on-surface-variant">Horário</p>
              <p class="mt-1 font-medium">${escapeHtml(timeText)}</p>
            </div>
            <div>
              <p class="text-xs font-medium text-on-surface-variant">Local</p>
              <p class="mt-1 font-medium">${escapeHtml(locationText)}</p>
            </div>
            <div>
              <p class="text-xs font-medium text-on-surface-variant">Formato</p>
              <p class="mt-1 font-medium">${escapeHtml(event.format)}</p>
            </div>
            <div>
              <p class="text-xs font-medium text-on-surface-variant">Área responsável</p>
              <p class="mt-1 font-medium">${escapeHtml(event.responsibleArea)}</p>
            </div>
            <div>
              <p class="text-xs font-medium text-on-surface-variant">Responsável principal</p>
              <p class="mt-1 font-medium">${escapeHtml(event.mainResponsible)}</p>
            </div>
            <div>
              <p class="text-xs font-medium text-on-surface-variant">Acessibilidade</p>
              <p class="mt-1 font-medium">A definir</p>
            </div>
            <div>
              <p class="text-xs font-medium text-on-surface-variant">ID</p>
              <p class="mt-1 font-medium">${escapeHtml(event.id)}</p>
            </div>
          </div>
        ...
        <section class="grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-6">
          <div class="border border-outline-variant bg-white p-4">
            <p class="text-xs font-medium uppercase tracking-wide text-on-surface-variant">Pendências</p>
            <p class="mt-2 text-2xl font-bold">${pending}</p>
            <p class="mt-1 text-xs text-amber-600">Abertas</p>
          </div>
          <div class="border border-red-200 bg-red-50 p-4">
            <p class="text-xs font-medium uppercase tracking-wide text-red-800">Atrasadas</p>
            <p class="mt-2 text-2xl font-bold text-red-900">${atrasadas}</p>
            <p class="mt-1 text-xs text-red-700">${atrasadas ? "Crítico" : "Sem atraso"}</p>
          </div>
          <div class="border border-outline-variant bg-white p-4">
            <p class="text-xs font-medium uppercase tracking-wide text-on-surface-variant">Próx. prazo</p>
            <p class="mt-2 text-lg font-bold">${escapeHtml(event.criticalDeadline || "A definir")}</p>
            <p class="mt-1 text-xs text-on-surface-variant">${escapeHtml(event.criticalArea || "A definir")}</p>
          </div>
          <div class="border border-outline-variant bg-white p-4">
            <p class="text-xs font-medium uppercase tracking-wide text-on-surface-variant">Comunicação</p>
            <p class="mt-2 text-lg font-bold text-emerald-700">${escapeHtml(communication)}</p>
            <p class="mt-1 text-xs text-on-surface-variant">Fluxo inicial</p>
          </div>
          <div class="border border-outline-variant bg-white p-4">
            <p class="text-xs font-medium uppercase tracking-wide text-on-surface-variant">Docs pendentes</p>
            <p class="mt-2 text-2xl font-bold">0</p>
            <p class="mt-1 text-xs text-on-surface-variant">A mapear</p>
          </div>
          <div class="border border-outline-variant bg-white p-4">
            <p class="text-xs font-medium uppercase tracking-wide text-on-surface-variant">Fornecedores</p>
            <p class="mt-2 text-2xl font-bold">0</p>
            <p class="mt-1 text-xs text-on-surface-variant">A mapear</p>
          </div>
        </section>
      ...
      <aside class="space-y-6">
        <section class="rounded-lg border border-outline-variant bg-white p-4">
          <h3 class="mb-3 text-xs font-medium uppercase tracking-wide text-on-surface-variant">Ações rápidas</h3>
          <div class="space-y-2 text-sm">
            <button class="flex w-full items-center gap-2 px-2 py-2 text-left hover:bg-surface-container"><span class="material-symbols-outlined text-base">add_task</span>Adicionar tarefa</button>
            <button class="flex w-full items-center gap-2 px-2 py-2 text-left hover:bg-surface-container"><span class="material-symbols-outlined text-base">person_add</span>Adicionar convidado</button>
            <button class="flex w-full items-center gap-2 px-2 py-2 text-left hover:bg-surface-container"><span class="material-symbols-outlined text-base">attach_file</span>Vincular documento</button>
            <button class="flex w-full items-center gap-2 px-2 py-2 text-left hover:bg-surface-container"><span class="material-symbols-outlined text-base">edit_note</span>Registrar atualização</button>
            <button class="mt-2 flex w-full items-center gap-2 border-t border-outline-variant px-2 py-3 text-left font-medium text-primary"><span class="material-symbols-outlined text-base">send</span>Enviar p/ Comunicação</button>
          </div>
        </section>

        <section class="rounded-lg border border-outline-variant bg-white p-4">
          <h3 class="mb-4 text-xs font-medium uppercase tracking-wide text-on-surface-variant">Equipe alocada</h3>
          <div class="space-y-3">
            <div class="flex items-center gap-3">
              <div class="flex h-8 w-8 items-center justify-center rounded-full bg-primary text-xs font-bold text-white">${escapeHtml(event.mainResponsible).slice(0, 2).toUpperCase()}</div>
              <div>
                <p class="text-sm font-medium">${escapeHtml(event.mainResponsible)}</p>
                <p class="text-xs text-on-surface-variant">Responsável principal</p>
              </div>
            </div>
            <div class="flex items-center gap-3">
              <div class="flex h-8 w-8 items-center justify-center rounded-full bg-surface-container text-xs font-bold text-on-surface-variant">EQ</div>
              <div>
                <p class="text-sm font-medium">Equipe a confirmar</p>
                <p class="text-xs text-on-surface-variant">${escapeHtml(event.responsibleArea)}</p>
              </div>
            </div>
          </div>
        </section>
